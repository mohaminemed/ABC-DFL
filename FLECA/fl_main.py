...
def initialize_attack_clients(num_groups, k, attack_name):
    ...
    for i in range(num_groups):
        selected_clients = (
            ...
            if i in [num_groups - 1] 
            else [i * k + (k - 1),  i * k + (k - 2)] #
        )
...
def FL_main_Loop(config, clients_data, sequences, sequences_test, clean_sequences_test , anomaly_labels_test, capacity_labels_test, attack_name, backdoored_test_indices):
    ...
    for round_num in range(config["num_rounds"]):
        ...
        for client_id, client_data in enumerate(clients_data):
          ...
          if len(client_data) > 0 :
           if attack_name == "gausslf" and client_id in attack_client_ids:
             client_weight = fl_training.train_client(
             local_model, client_data, optimizer_class, optimizer_reg,
             criterion_class, criterion_reg, config, global_model, round_num,"gausslf"
                )
             print(f"gausslf: {client_id}")

           elif attack_name == "magnitude" and client_id in attack_client_ids:
             client_weight = fl_training.train_client(
             local_model, client_data, optimizer_class, optimizer_reg,
             criterion_class, criterion_reg, config, global_model, round_num,"magnitude"
                )
             print(f"magnitude: {client_id}")  

           elif attack_name == "trim" and client_id in attack_client_ids:
             client_weight = fl_training.train_client(
             local_model, client_data, optimizer_class, optimizer_reg,
             criterion_class, criterion_reg, config, global_model, round_num,"trim"
                )
             print(f"trim: {client_id}")  

           elif attack_name == "krum" and client_id in attack_client_ids:
             client_weight = fl_training.train_client(
             local_model, client_data, optimizer_class, optimizer_reg,
             criterion_class, criterion_reg, config, global_model, round_num,"krum"
                )
             print(f"krum: {client_id}")    

           elif attack_name == "backdoor" and client_id in attack_client_ids:
             client_weight = fl_training.train_client(
             local_model, client_data, optimizer_class, optimizer_reg,
             criterion_class, criterion_reg, config, global_model, round_num,"backdoor"
                )
             print(f"backdoor: {client_id}")   

           else:
              client_weight = fl_training.train_client(
              local_model, client_data, optimizer_class, optimizer_reg,
              criterion_class, criterion_reg, config, global_model, round_num,"correct"
                )
    ...
    results_file_path = f"./Results/MaliciousRate/{config['model_name']}_IID_{config['iid']}_{config['aggregation']}_{config['num_clients']}_Client__{config['num_rounds']}_Rounds.csv"
