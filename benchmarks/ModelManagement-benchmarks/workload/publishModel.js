...
class StoreWorkload extends WorkloadModuleBase {
     ...
     */
     _generateWorkload() {
        ...
        if(!isInit){
            isInit = true;
            var data = fs.readFileSync("./workload/scripts/modelData.json");
        ...
        for(let i= 0 ; i <  this.roundArguments.txnPerBatch ; i++) {
            this.txIndex++; 
            let _modelCID = myArray[this.txIndex+roundIndexes[this.roundArguments.num-1]]._modelCID ; 
            ...
            let requiredCSs = myArray[this.txIndex+roundIndexes[this.roundArguments.num-1]].requiredTrainers ; 
            let args = {
                contract :this.roundArguments.contract,
                verb: 'publishModel',
                args: [_modelCID,_infoCID,maxRounds,requiredCSs],
                readOnly: false,
